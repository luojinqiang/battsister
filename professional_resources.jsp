...
<%
	...
	List<Doc> newslist=selectic.Get_List(pages, pn, counts, table, where, file, "order by is_top desc,ordernum asc ", "mysqlss",valuelist);
...
<div class="container">
   ...
   <%
		if(newslist!=null){
			for(Doc doc:newslist){
				%>
				<div class="news">
			        <div class="news_img"><a href="news_details.jsp?news_id=<%=doc.getIn("id")%>&type=5&other_type=<%=type%>&pages=<%=pages%>"><img src="<%=doc.get("smallfile")%>"></a></div>	
			        <div class="news_word">
			          <h3><a href="news_details.jsp?news_id=<%=doc.getIn("id")%>&type=5&other_type=<%=type%>&pages=<%=pages%>"><%=doc.get("newstitle")%></a></h3>
			          <div class="time"><%=AjaxXml.timeStamp2Date(doc.getIn("addtime"),"YY04-MM-DD")%></div>
			          <p><%=doc.get("note")%></p>
			            <div class="news_botton"><a href="news_details.jsp?news_id=<%=doc.getIn("id")%>&type=5&other_type=<%=type%>&pages=<%=pages%>">查看详情</a></div>
			        </div>
			        <div class="clear"></div>
  			 	</div>
				<%
  ...
  <!-- 分页 -->
     <ul class="pre">
      <%=page_size>0?"<li><a href=\""+(pages!=1?"professional_resources.jsp?pages="+(pages-1)+"&type="+type:"javascript:void(0);")+"\"><</a></li>":""%>
       <%
       	int index=1;
       	int index2=page_size;
       	if(page_size>=5){
       	...
       	for(int i=index;i<=index2;i++){
       		out.print("<li"+(i==pages?" class=\"active_pre\"":"")+"><a href=\"professional_resources.jsp?pages="+i+"&type="+type+"\">"+i+"</a></li>");
       	}
       %>
       <%=page_size>0?"<li><a href=\""+(pages!=page_size?"professional_resources.jsp?pages="+(pages+1)+"&type="+type:"javascript:void(0);")+"\">></a></li>":""%>
    </ul>
</div>
<!-- 引入尾部 -->
