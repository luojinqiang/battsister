...
public class UserMember {
     ...
     * @return
     */
    public JSONObject memDel(HttpServletRequest request, int userid, String username, int gym_group_id, int gym_id) {
        Dbc dbc = DbcFactory.getBbsInstance();
        Base base = new Base();
        JSONObject backjson = new JSONObject();
        String ajaxRequest = "";
        String logtitle = "会员-删除";
        try {
            ...
            int id = ru.getInt("id");
            Doc doc = base.executeQuery2Docs("select id from hy_member where id=? and isdel=0", new Object[]{id}, 1)[0];
            if (doc == null || doc.isEmpty()) {
                backjson.put("type", false);
                backjson.put("msg", "会员不存在");
                return backjson;
            }
            base.executeUpdate("update hy_member set isdel=1 where id=?", new Object[]{id});
     ...
     * @return
     */
    public JSONObject memDelBatch(HttpServletRequest request, int userid, String username, int gym_group_id, int gym_id) {
        Dbc dbc = DbcFactory.getBbsInstance();
        Base base = new Base();
        JSONObject backjson = new JSONObject();
        String ajaxRequest = "";
        String logtitle = "会员-批量删除";
        try {
            dbc.openConn("mysqlss");
            base.setDbc(dbc, false);
            ajaxRequest = AjaxXml.getParameterStr(request);
            ...
            base.executeUpdate("update hy_member set isdel=1 where id in (" + wenhao + ")", sqllist);
            Logdb.WriteSysLog(gym_group_id, gym_id, ajaxRequest, logtitle, username, userid, ru.getIps(), 0, base);
            base.commit();
            backjson.put("type", true);
            backjson.put("msg", "操作成功");
