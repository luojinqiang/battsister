...
public class Course {
    ...
    public JSONObject editVideo(HttpServletRequest request, int userid, String username) {
        ...
        try {
            ...
            base.executeUpdate("update bs_course set practical_video_path=? where id=? ", new Object[]{filePath.toString(), id});
    ...
    public static JSONArray genFilePath(String filePath_str, String title_str, String order_no_str) throws Exception {
        ...
        if (filePath_str != null && !"".equals(filePath_str)) {
            filePaths = filePath_str.split(",");
            if (title_str != null) {
                titles = title_str.split(",");
            }
            if (order_no_str != null && !"".equals(order_no_str)) {
                orderNos = order_no_str.split(",");
            }
            JSONObject json;
            for (int i = 0; i < filePaths.length; i++) {
                ...
                if (!"".equals(filePaths[i].trim())) {
                    ...
                    if (orderNos != null && orderNos.length > i) {
                        json.put("order_no", orderNos[i]);
                    } else {
                        json.put("order_no", 0);
                    }
                    filePath.add(json);
