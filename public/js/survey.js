...
var survey_obj={
	survey_edit_init:function(){
		global_obj.file_upload($('#ReplyImgUpload'), $('#survey_form input[name=ReplyImgPath]'), $('#ReplyImgDetail'));
		$('#ReplyImgDetail').html(global_obj.img_link($('#survey_form input[name=ReplyImgPath]').val()));
		...
		$('#survey_form input:submit').click(function(){
			...
			$.post('?', $('#survey_form').serialize()+'&contents='+encodeURIComponent(editor.getContent()), function(data){
				if(data.ret==1){
					window.location='/member/survey/survey.jsp';
				}else{
	...
	survey_set_init:function(){
		...
		$('#survey_list a[href=#survey_mod_list]').each(function(){
			$(this).click(function(){
				...
				.end().filter('[name=qid]').val(jsonData.qid)
		...
		$('#survey_list a[href=#survey_del_list]').each(function(){
			$(this).click(function(){
				...
				var jsonData=eval('('+$(this).parent().attr('Data')+')');
				$.post('?', "ajax=1&do_action=survey.survey_list_del&qid="+jsonData.qid+"&LId="+jsonData.LId, function(data){
					if(data.status==1){
						window.location.reload();
					}else{
						alert('出现未知错误！');
		...
		$('#survey_list a[href=#survey_add_answer]').click(function(){
			...
			$('#survey_answer_add_form input').filter('[name=qid]').val(jsonData.qid)
			.end().filter('[name=LId]').val(jsonData.LId);
		});
		...
		$('#survey_answer_add_form input:submit').click(function(){
			...
			$.post('?', $('#survey_answer_add_form').serialize(), function(data){
				if(data.status==1){
					...
					$('#survey_answer_add_form .submit').attr('disabled', false);
					alert('添加答案失败，出现未知错误！');
				};
			}, 'json');
		});
		...
		$('#survey_list a[href=#survey_del_answer]').each(function(){
			$(this).click(function(){
				...
				var jsonData=eval('('+$(this).parent().parent().attr('DataAnswer')+')');
				$.post('?', "ajax=1&do_action=survey.survey_answer_del&qid="+jsonData.qid+"&LId="+jsonData.LId+"&aid="+jsonData.aid, function(data){
					if(data.status==1){
						window.location.reload();
					}else{
						alert('出现未知错误！');
