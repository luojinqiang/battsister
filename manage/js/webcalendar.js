...
function SelectDate(obj, strFormat, x, y) {
    ...
    var ey = date.getFullYear() + 15; //最大值 → 50 年后
...
Calendar.prototype.draw = function () {
    calendar = this;
    var mvAry = [];
//mvAry[mvAry.length] = ' <form name="calendarForm" style="margin: 0px;">'; //因 <form> 不能嵌套， 用 Div 
...
//mvAry[mvAry.length] = ' </from>'; 
    ...
    obj.onclick = function () {
        calendar.ReturnDate("");
        /*calendar.dateControl.value = "";calendar.hide();*/
    }
    ...
    obj.onclick = function () {
        var xday = calendar.day;
        if (calendar.DateMode >= pickMode["month"]) {
            xday = "01";
        }
        var today = (calendar.DateMode != pickMode["day"]) ?
            new Date(calendar.year, calendar.month, xday, calendar.hour, calendar.minute, calendar.second)
            : new Date();//2008-01-29
