...
<%
    ...
    if (action.equals("save")) {//更新
    	...
    	out.print(course.editTeachingData(request, user_id, user_name));
    	return;
    }
	String teaching_data="",name="";
    Doc doc = utildb.Get_Doc("id,teaching_data,name", "bs_course", " where id=? and isdel=0", "mysqlss", new Object[]{id});
    if (doc == null) {
        out.print("信息不存在");
        return;
    } else {
    	teaching_data=doc.get("teaching_data");
    	name=doc.get("name");
    	}
%>
...
<%
	...
	if(teaching_data!=null&&!"".equals(teaching_data)){
		JSONObject path_json=JSONObject.fromObject(teaching_data);
		if(path_json!=null){
    ...
    <div class="box_input">
        <form id="form1" name="form1" method="post" action="">
            ...
            <input name="action" id="action" type="hidden" value="save"/>
                ...
                <li>
                       <h4 style="margin-bottom:20px;"><%=name%>--教材文档上传</h4>
                </li>
            </ul>
            <ul class="row1 clearfix">
                <li>
