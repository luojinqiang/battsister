...
<%
    ...
    if (action.equals("save")) {//更新
    	...
    	out.print(course.editTeachingEvalution(request, user_id, user_name));
    	return;
    }
	String teaching_evalution="",name="";
    Doc doc = utildb.Get_Doc("id,teaching_evalution,name", "bs_course", " where id=? and isdel=0", "mysqlss", new Object[]{id});
    if (doc == null) {
        out.print("信息不存在");
        return;
    } else {
    	teaching_evalution=doc.get("teaching_evalution");
    	name=doc.get("name");
    	}
%>
    ...
    <script type="text/javascript">
        function usersave() {
            ...
            $.ajax({
                dataType: "json",
                type: "post",
                url: "teaching_evalution_edit.jsp",
...
<%
	...
	if(teaching_evalution!=null&&!"".equals(teaching_evalution)){
		JSONArray pathArray=JSONArray.fromObject(teaching_evalution);
		if(pathArray!=null){
    ...
    <div class="box_input">
        <form id="form1" name="form1" method="post" action="">
            ...
            <input name="action" id="action" type="hidden" value="save"/>
                ...
                <li>
                       <h4 style="margin-bottom:20px;"><%=name%>--教学评价文档上传</h4>
                </li>
            </ul>
            <ul class="row1 clearfix">
                <li>
