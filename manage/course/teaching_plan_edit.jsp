...
<%
    ...
    if (action.equals("save")) {//更新
    	...
    	out.print(course.editTeachingPlan(request, user_id, user_name));
    	return;
    }
	String teaching_plan="",name="";
    Doc doc = utildb.Get_Doc("id,teaching_plan,name", "bs_course", " where id=? and isdel=0", "mysqlss", new Object[]{id});
    if (doc == null) {
        out.print("信息不存在");
        return;
    } else {
    	teaching_plan=doc.get("teaching_plan");
    	name=doc.get("name");
    	}
%>
...
<%
	...
	if(teaching_plan!=null&&!"".equals(teaching_plan)){
		JSONObject path_json=JSONObject.fromObject(teaching_plan);
		if(path_json!=null){
